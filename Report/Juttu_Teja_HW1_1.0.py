...
def compress_img(img, clusters):
    ...
    #Random initialization of centroids from RGB colorspace
    ...
    for iter in range(0, iter_max):
        ...
        #print(centroids)
        if np.array_equal(count_old,count):
            total_iters = iter
            break
        total_iters = iter
    ...
    plt.show()
    #plt.imsave('GTechcompressed_' + str(clusters) +'clusters_'+str(total_iters)+'iters.png', imgout)
...
L,k =compress_img(asarray(Image.open('GeorgiaTech.bmp')),4)   
print(L,k)

     #Uncomment the below to look at the Output for HW Q 1.1 & 1.2 
        # and return (runtime,total_iters) instead of labels and centroids
