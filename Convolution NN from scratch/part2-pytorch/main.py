...
def train(epoch, data_loader, model, optimizer, criterion):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        #############################################################################
        # print(data.shape)
        optimizer.zero_grad() #Refreshing gradients to 0 for every iteration over a batch 
        out = model(data) 
        loss = criterion(out, target)
        loss.backward() # Gradients computing
...
def validate(epoch, val_loader, model, criterion):
    ...
    for idx, (data, target) in enumerate(val_loader):
        ...
        #       HINT: torch.no_grad()                                               #
        #############################################################################
        with torch.no_grad(): # No backprop. Memory optimization
            out = model(data)
            loss = criterion(out, target)
        #############################################################################
        #                              END OF YOUR CODE                             #
